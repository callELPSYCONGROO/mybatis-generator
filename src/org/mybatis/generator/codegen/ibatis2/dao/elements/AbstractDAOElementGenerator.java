...
public abstract class AbstractDAOElementGenerator extends AbstractGenerator {
    ...
    private JavaVisibility criteriaMethodVisibility;
    ...
    public JavaVisibility getCriteriaMethodVisibility() {
        if (criteriaMethodVisibility == null) {
            String type = context
                    .getJavaClientGeneratorConfiguration()
                    .getProperty(PropertyRegistry.DAO_EXAMPLE_METHOD_VISIBILITY);
            if (stringHasValue(type)) {
                if ("public".equalsIgnoreCase(type)) { //$NON-NLS-1$
                    criteriaMethodVisibility = JavaVisibility.PUBLIC;
                } else if ("private".equalsIgnoreCase(type)) { //$NON-NLS-1$
                    criteriaMethodVisibility = JavaVisibility.PRIVATE;
                } else if ("protected".equalsIgnoreCase(type)) { //$NON-NLS-1$
                    criteriaMethodVisibility = JavaVisibility.PROTECTED;
                } else if ("default".equalsIgnoreCase(type)) { //$NON-NLS-1$
                    criteriaMethodVisibility = JavaVisibility.DEFAULT;
                } else {
                    criteriaMethodVisibility = JavaVisibility.PUBLIC;
        ...
        return criteriaMethodVisibility;
    }
}
